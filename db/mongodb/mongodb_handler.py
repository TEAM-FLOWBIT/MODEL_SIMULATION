...
class MongoDBHandler(DBHandler):
    ...
    def __init__(self, mode="remote", db_name=None, collection_name=None):
        ...
        #print(self.db_config.get("user"))
        #print(self.db_config.get("password"))
    ...
    def find_items_for_db(self, condition=None, db_name=None, collection_name=None):
        """
        MongdDB에서 하나의 문서를 검색하기 의한 메소드입니다.
        :param condition: 검색 조건을 딕셔너리 형태로 받는다
        :param db_name: MongoDB에서 데이터베이스에 해당하는 이름을 받는다
        ...
        if collection_name is not None:
            self._collection = self._db[collection_name]
        return self._collection.find().sort("_id", -1).limit(15)
    
    def find_items_for_chart(self, condition=None, db_name=None, collection_name=None, limit=None):
        """
        MongdDB에서 하나의 문서를 검색하기 의한 메소드입니다.
        :param condition: 검색 조건을 딕셔너리 형태로 받는다
        :param db_name: MongoDB에서 데이터베이스에 해당하는 이름을 받는다
